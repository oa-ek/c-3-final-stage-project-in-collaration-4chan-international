...
public static class DataInitializer
{
    public static async Task InitializeAsync(IServiceProvider serviceProvider)
    {
        ...
        if (!await context.WeaponTypes.AnyAsync())
        {
            ...
            var typeGreatAxe = new WeaponType { Name = "Great_Axe", SlotId = slotRHandId };
        ...
        if (!await context.ArmorTypes.AnyAsync())
        {
        ...
        {
            var typeHugeId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Huge sword")).Id;
            var typeGreatSwordId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Greatsword")).Id;
            var typeStraightSwordId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Straight sword")).Id;
            var typeHalberdId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Halberd")).Id;
            var typeSpearId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Spear")).Id;
            var typeKatanaId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Katana")).Id;
            var typeShieldId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Shield")).Id;
            var typeGreatAxeId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Great_Axe")).Id;
            var typeDaggerId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Dagger")).Id;
            var typeCurvedSwordId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Curved Sword")).Id;
            var typeThrustingId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Thrusting Sword")).Id;
            var typeAxeId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Axe")).Id;
            var typeHammerId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Hammer")).Id;
            var typeGreatHammerId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Great Hammer")).Id;
            var typeCurvedGreatId = (await context.WeaponTypes.FirstAsync(wt => wt.Name == "Curved Greatsword")).Id;
            ...
            var weapons = new List<WeaponEquipment>
            {
                ...
                new()
                {
                    Name="Dark Sword",
                    Weight=6.0m,
                    WeaponTypeId = typeStraightSwordId,
                    ReqStrength=16,
                    ReqDexterity=16,
                ...
                new()
                {
                    Name = "Falchion",
                    Weight = 2.5m,
                    WeaponTypeId = typeCurvedSwordId,
                    ReqStrength = 9,
                    ReqDexterity = 13,
                ...
                new()
                {
                    Name = "Dragon Tooth",
                    Weight = 18.0m,
                    WeaponTypeId = typeGreatHammerId,
                    ReqStrength = 40,
                    ReqDexterity = 0,
                ...
                new()
                {
                    Name = "Murakumo",
                    Weight = 12.0m,
                    WeaponTypeId = typeCurvedGreatId,
                    ReqStrength = 28,
                    ReqDexterity = 13,
        ...
        if (!await context.Armors.AnyAsync())
        {
            var typeHelmId = (await context.ArmorTypes.FirstAsync(at => at.Name == "Helm")).Id;
            var typeChestArmorId = (await context.ArmorTypes.FirstAsync(at => at.Name == "Chest ArmorEquipment")).Id;
            var typeGauntletsId = (await context.ArmorTypes.FirstAsync(at => at.Name == "Gauntlets")).Id;
            var typeLeggingsId = (await context.ArmorTypes.FirstAsync(at => at.Name == "Leggings")).Id;
